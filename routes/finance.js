...
app.get('/getFinances', (req, res, next) => {
  db.query("SELECT o.project_id,o.project_type,o.creation_date,amount,order_status,invoice_terms,notes,shipping_first_name,shipping_address1,shipping_address2,shipping_address_country,delivery_date,delivery_terms,company_name,cust_address1,cust_address2,cust_address_country,cust_address_po_code,,gc2.name AS shipping_country_name,c.company_name AS company_name,c.website AS company_website,c.fax AS company_fax ,c.phone AS company_phone,c.address_flat AS company_address_flat ,c.address_street AS company_address_street,c.address_town AS company_address_town,c.address_state AS company_address_state ,gc3.name AS company_country_name,(SELECT (SUM(oi.unit_price * oi.qty) + o.shipping_charge) FROM order_item oi WHERE oi.order_id = o.order_id) AS order_amount,q.quote_code,p.project_code FROM `order` o LEFT JOIN geo_country gc2 ON (o.shipping_address_country_code = gc2.country_code) LEFT JOIN company c ON (o.company_id = c.company_id) LEFT JOIN geo_country gc3 ON (c.address_country = gc3.country_code) LEFT JOIN quote q ON o.quote_id = q.quote_id LEFT JOIN project p ON o.project_id = p.project_id WHERE o.order_id !='' ",
    (err, result) => {
